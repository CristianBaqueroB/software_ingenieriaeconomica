...
class PresentValueGeometricGradientEqualPage extends StatefulWidget {
  const PresentValueGeometricGradientEqualPage({super.key});

  @override
  _PresentValueGeometricGradientEqualPageState createState() => _PresentValueGeometricGradientEqualPageState();
}

class _PresentValueGeometricGradientEqualPageState extends State<PresentValueGeometricGradientEqualPage> {
  ...
  void _calculatePresentValue() {
    ...
    if (initialPayment != null && gradient != null && time != null && interestRate != null) {
      ...
      if (G == i) {
        // Calcular el valor presente usando la fórmula específica para G = i
        final term1 = initialPayment * n;
        final term2 = 1 / pow(1 + i, n);
        final presentValue = term1 * term2;
        ...
        // Mostrar mensaje de error si G es diferente a i
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Error: El gradiente (G) debe ser igual a la tasa de interés (i).'),
          ),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Valor Presente Gradiente Geométrico (G = i)"),
      ),
